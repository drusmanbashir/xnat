...
if not logger.handlers:
    fh = logging.FileHandler(LOG_DIR / "xnat_bad_folders.log")
...
def login():
    ...
    central = Interface(
        server=xnat_settings["server"],
        user=xnat_settings["user"],
        password=xnat_settings["password"],
    )
    ...
    def add_rsc(self, fpath, label, content="CT", format="NIFTI", tags=[], force=False):
        ...
        if not self.has_rsc(label):
            ...
            if len(tags) > 0:
                for tg in tags:
                    rsc_neo.tag(tg)
            rsc_neo.file(fname).insert(fpath, content=content, format=format)
            # rsc_neo.create()
            print("Resource labelled {0} added from filepath: {1}".format(label, fpath))
        else:
            print("Resources labelled {} already exists. Nothing to do.".format(label))
...
class Proj(_BaseObj):
    def __init__(self, proj_title: str, lazy_load=True):
        ...
        # With lazy_load=True, subjects are not loaded until needed

    def get_subs_all(self):
        """Get all subjects, with caching for better performance"""
        if self._subjects_cache is None:
            subs = self.esp.subjects()
            self._subjects_cache = [Subj(scn) for scn in subs]
    ...
    def _cp_files(self, files, fldr, fnc, overwrite):
        for f in files:
            ...
            if overwrite == True and outfname.exists():
                os.remove(outfname)
            try:
    ...
    def keywords(self):
        c, _ = login()
        const = [("xnat:projectData/name", "=", self.name)]
        aa = c.select(
            "xnat:projectData",
            [
...
class Subj(GetAttr):
    ...
    def get_rscs(self):
        ...
        rscs = list(il.chain.from_iterable(r))
        return rscs
    ...
    @property
    def exp_ids(self):
        if not hasattr(self, "_exp_ids"):
            self._exp_ids = self.scn.experiments().get()
    ...
    @property
    def rscs(self):
        if not hasattr(self, "_rscs"):
            self._rscs = self.get_rscs()
        return self._rscs
...
class Exp(_ExpScn):
    def __init__(self, pt_id, esp: Experiment):
        assert isinstance(
            esp, Experiment
        ), "Initialize with a Experiment instance please"
...
class Scn(_ExpScn):
    ...
    def parent(self):
        return Exp(self.pt_id, self.esp.parent())
    ...
    def nii_postprocess(self, rsc_label: str, output_label: str, fnc, *fncargs: None):
        ...
        try:
            assert len(fpath) == 1, "More than one nii file with label: {0}".format(
                rsc_label
            )
            fpath = Path(fpath[0])
        ...
        try:
            assert (
                self.has_rsc(output_label) == False
            ), "Resource {} already exists. Skipping..".format(output_label)
    ...
    def download_dcm_fns(self):
        ...
        assert len(rsc_dcm) == 1, "More than one DICOM resource found for {0}".format(
            self.pt_id
        )
        rsc = rsc_dcm[0]
    ...
    def _sitk_convert(self, dcm_fn):
        ...
        try:
            ...
            # keep your existing behaviour (re-raise or return None as you prefer)
            raise
...
class ScnSeg(Scn):
    ...
    def dcm2nii(self, label="LABELMAP", add_date=True, add_desc=True, overwrite=False):
        return super().dcm2nii(
            label=label, add_date=add_date, add_desc=add_desc, overwrite=overwrite
        )
...
def upload_nii_nodesc(fpath: Union[str, Path], label, tags: list = []):
    ...
    exps = fname_to_exp(str(fpath), has_date=False)
    if len(exps) != 1:
...
@str_to_path(0)
def get_matching_rsc(fpath, target_label="IMAGE", has_date=True):
